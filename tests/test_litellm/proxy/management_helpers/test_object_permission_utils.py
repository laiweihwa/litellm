...
# ---- VERIA-218: personal-key non-admin gates on toolsets / vector_stores / search_tools ----
...
async def test_veria_218_personal_non_admin_cannot_assign_mcp_toolsets(
    mock_access_groups, mock_allow_all
):
    with pytest.raises(HTTPException) as exc:
        await validate_key_mcp_servers_against_team(
...
async def test_veria_218_personal_admin_can_assign_mcp_toolsets(
    mock_access_groups, mock_allow_all
):
    await validate_key_mcp_servers_against_team(
        object_permission={"mcp_toolsets": ["ts-private"]},
...
@pytest.mark.asyncio
async def test_veria_218_personal_non_admin_cannot_assign_vector_stores():
    with pytest.raises(HTTPException) as exc:
...
@pytest.mark.asyncio
async def test_veria_218_personal_admin_can_assign_vector_stores():
    await validate_key_vector_stores_against_team(
        object_permission={"vector_stores": ["vs-private"]},
        team_obj=None,
        is_proxy_admin=True,
...
@pytest.mark.asyncio
async def test_veria_218_team_key_vector_stores_unrestricted_at_create():
    """Team keys are out of VERIA-218 scope; existing trust model preserved."""
...
@pytest.mark.asyncio
async def test_veria_218_personal_non_admin_cannot_assign_search_tools():
    with pytest.raises(HTTPException) as exc:
        await validate_key_search_tools_against_team(
...
@pytest.mark.asyncio
async def test_veria_218_personal_admin_can_assign_search_tools():
    await validate_key_search_tools_against_team(
        object_permission={"search_tools": ["st-private"]},
        team_obj=None,
        is_proxy_admin=True,
...
@pytest.mark.asyncio
async def test_veria_218_empty_object_permission_passes_for_personal_non_admin():
