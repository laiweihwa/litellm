...
async def test_veria_218_generate_key_personal_non_admin_denied(
    monkeypatch, field, request_kwargs, expected_in_error
):
    """VERIA-218: generate_key_fn must reject access_group_ids and
...
@pytest.mark.asyncio
async def test_veria_218_update_key_personal_non_admin_denied(monkeypatch):
    """VERIA-218: /key/update must reject vector_stores on a personal key by a
    non-admin. Reverting the enforce_member_can_assign_access_groups move (i.e.
    putting it back inside `if _team_id_to_check is not None`) does NOT cover
...
@pytest.mark.asyncio
async def test_veria_218_update_key_personal_non_admin_access_groups_denied(
    monkeypatch,
):
    """VERIA-218: /key/update on a personal key must also gate access_group_ids
    for non-admins. Reverting the enforce move (putting it back inside
